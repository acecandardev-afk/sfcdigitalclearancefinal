...
const NAV = [
  ...
  { path: '/dashboard/clearances/report', label: 'Generate Report', icon: FileCog },
] as const;
  ...
  switch (item.path) {
    ...
    case '/dashboard/clearances':
      ...
      if (pathname.startsWith('/dashboard/clearances/')) {
        ...
        if (seg === 'calendar' || seg === 'report') return false;
        return true;
      }
      return false;
    default:
...
export default function StudentSidebar({ onNavigate }: StudentSidebarProps) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[#f8fafc] text-slate-800 dark:bg-slate-950 dark:text-slate-100">
      {/* Brand header */}
      <div className="shrink-0 bg-[#1e3a5f] px-4 py-5 text-white">
        <div className="flex items-start gap-3">
          <div className="h-12 w-12 shrink-0 overflow-hidden rounded-full border border-white/20 bg-white/10">
            <img src="/logo4.png" alt="" className="h-full w-full object-contain p-1" />
          </div>
          <div className="min-w-0 pt-0.5">
            <p className="text-lg font-bold leading-tight tracking-tight">E-Clear SFCG</p>
            <p className="mt-1 text-xs font-medium text-sky-100/90">Saint Francis College — Guihulngan</p>
            <p className="mt-0.5 text-[11px] text-sky-100/75">Student Clearance System</p>
          </div>
        </div>
      </div>
      ...
      <div className="shrink-0 border-b border-slate-200/80 px-4 py-4 dark:border-slate-800">
        ...
        <div className="mt-3 flex items-center gap-3">
          <Avatar className="h-11 w-11 border border-slate-200 dark:border-slate-700">
            <AvatarFallback className="bg-[#1e3a5f] text-sm font-bold text-white">
      ...
      <nav className="min-h-0 flex-1 space-y-1 overflow-y-auto px-3 py-4">
        <p className="px-2 pb-2 text-[10px] font-semibold uppercase tracking-wider text-slate-400">Navigation</p>
        {NAV.map((item) => {
            ...
            <Link
              ...
              className={cn(
                ...
                active
                  ...
                  : 'text-slate-700 hover:bg-white hover:text-slate-900 dark:text-slate-200 dark:hover:bg-slate-900'
              )}
            >
      ...
      <div className="shrink-0 border-t border-slate-200/80 px-4 py-3 dark:border-slate-800">
        <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Appearance</p>
        <div className="mt-2 grid grid-cols-3 gap-1 rounded-xl border border-slate-200/90 bg-white p-1 dark:border-slate-700 dark:bg-slate-900">
          {(
            [
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  active
                    ? 'bg-white text-[#1e3a5f] shadow-sm ring-1 ring-slate-200 dark:bg-slate-800 dark:text-sky-300 dark:ring-slate-600'
      ...
      <div className="shrink-0 px-4 pb-3">
        <div className="rounded-xl bg-sky-100/80 px-3 py-3 dark:bg-sky-950/40">
          <p className="text-sm font-semibold text-[#1e3a5f] dark:text-sky-300">Need assistance?</p>
