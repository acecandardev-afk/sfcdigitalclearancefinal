...
interface InstitutionalClearanceLike {
  createdAt: string;
  status: 'draft' | 'pending' | 'in_progress' | 'completed' | 'rejected';
}

interface InstitutionalClearanceInsightsProps {
  clearances: InstitutionalClearanceLike[];
}
...
export default function InstitutionalClearanceInsights({ clearances }: InstitutionalClearanceInsightsProps) {
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <CardDescription className="text-xs mt-0.5">
                Requests submitted per month (last 6 months)
              </CardDescription>
            </div>
          </div>
        </CardHeader>
            ...
            {hasAny ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={activitySeries} margin={{ top: 8, right: 8, left: -8, bottom: 0 }}>
                  <defs>
                    <linearGradient id="fillInstitutionalSubmissions" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Tooltip contentStyle={tipStyle} labelStyle={{ fontWeight: 600, marginBottom: 4 }} formatter={(value: number | string) => [value, 'Submissions']} />
                  <Area
                    type="monotone"
                    dataKey="submissions"
                    name="Submissions"
                    stroke={PRIMARY}
                    strokeWidth={2}
                    fill="url(#fillInstitutionalSubmissions)"
        ...
        <CardHeader className="pb-2 border-b border-border/40 bg-muted/20">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <CardDescription className="text-xs mt-0.5">
                How request statuses are distributed each month
              </CardDescription>
            </div>
          </div>
        </CardHeader>
            ...
            {hasAny ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={monthlyStacked} margin={{ top: 8, right: 8, left: -8, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" className="stroke-border/60" vertical={false} />
                  <XAxis dataKey="name" tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }} axisLine={false} tickLine={false} />
                  <YAxis allowDecimals={false} width={32} tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }} axisLine={false} tickLine={false} />
                  <Tooltip contentStyle={tipStyle} labelStyle={{ fontWeight: 600 }} />
                  <Legend wrapperStyle={{ fontSize: 11, paddingTop: 8 }} />
                  <Bar dataKey="Pending" stackId="s" fill="hsl(38 92% 50%)" maxBarSize={48} />
                  <Bar dataKey="In Progress" stackId="s" fill="hsl(262 40% 55%)" maxBarSize={48} />
                  <Bar dataKey="Signed" stackId="s" fill="hsl(142 70% 42%)" maxBarSize={48} />
                  <Bar dataKey="Rejected" stackId="s" fill="hsl(var(--destructive))" maxBarSize={48} />
                  <Bar dataKey="Draft" stackId="s" fill="hsl(215 16% 56%)" radius={[6, 6, 0, 0]} maxBarSize={48} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-full flex flex-col items-center justify-center text-muted-foreground text-sm border border-dashed border-border rounded-lg">
